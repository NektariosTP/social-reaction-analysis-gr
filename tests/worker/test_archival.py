...
async def test_run_archival_sweep_returns_metrics() -> None:
    session = _mock_session_with_results([2, 1])

    metrics = await run_archival_sweep(session)

    assert metrics == {"n_archived": 2, "n_closed": 1}
    session.commit.assert_awaited_once()


async def test_run_archival_sweep_handles_zero_matches() -> None:
    session = _mock_session_with_results([0, 0])

    metrics = await run_archival_sweep(session)

    assert metrics == {"n_archived": 0, "n_closed": 0}
...
async def test_run_archival_sweep_uses_72_hour_and_10_day_thresholds() -> None:
    ...
    close_sql = str(session.execute.call_args_list[1][0][0])
    close_params = session.execute.call_args_list[1][0][1]
